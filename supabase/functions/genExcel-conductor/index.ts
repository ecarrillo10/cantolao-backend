...
const FOLDER = "Reportes/Conductores";
const SHEET_NAME = "Reporte por Conductor";
...
function autoFitColumns(ws, colCount) {
  for(let i = 1; i <= colCount; i++){
    ...
    ws.eachRow({
      ...
      const txt = typeof val === "object" ? "" : String(val);
      max = Math.max(max, txt.length);
    });
    ws.getColumn(i).width = Math.min(Math.max(max + 2, 14), 40);
  }
}
/* Fecha + hora Lima → DDMMYY_HHMMSS */ function nowLimaCompact() {
...
/* ================= SERVER ================= */ serve(async (req)=>{
  ...
  try {
    ...
    /* ---------- Auth ---------- */ const auth = req.headers.get("Authorization")?.replace("Bearer ", "");
    if (!auth) return json({
      error: "No autorizado"
    }, 401);
    ...
    /* ---------- Data (DESDE LA VISTA) ---------- */ const { data, error } = await sbSR.from("vw_reporte_vehiculo").select(`
        conductor,
        dni,
        ...
        vehiculo,
        total_monto
      `).eq("id_cliente", idCliente).order("conductor", {
      ascending: true
    });
    ...
    data.forEach((r)=>{
      const row = ws.addRow([
        r.conductor,
        r.dni,
        r.total_abastecimientos,
        Number(r.total_galones),
        r.vehiculo,
    ...
    ws.getColumn(6).numFmt = '"S/ " #,##0.00';
    autoFitColumns(ws, headers.length);
    /* ---------- Upload ---------- */ const fileName = `Reporte_Conductores_${nowLimaCompact()}.xlsx`;
