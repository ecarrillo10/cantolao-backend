...
const FOLDER = "Reportes/Vehiculos";
const SHEET_NAME = "Reporte por Vehículo";
...
/* ================= SERVER ================= */ serve(async (req)=>{
  ...
  try {
    ...
    /* ---------- Data ---------- */ const { data, error } = await sbSR.from("vw_reporte_vehiculo").select(`
        vehiculo,
        modelo,
        total_abastecimientos,
        total_galones,
        total_monto
      `).eq("id_cliente", idCliente).order("vehiculo", {
      ascending: true
    });
    ...
    /* ---------- Upload ---------- */ const fileName = `Reporte_Vehiculos_${nowLimaCompact()}.xlsx`;
