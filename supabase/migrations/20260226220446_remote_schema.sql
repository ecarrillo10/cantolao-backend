...
create or replace view "public"."vw_detalle_conductor" as  SELECT c.id_conductor,
    ...
    COALESCE(c.telefono, 'Sin información'::character varying) AS telefono,
    COALESCE(c.licencia, 'Sin información'::character varying) AS licencia,
    c.fecha_vencimiento_licencia,
    COALESCE(c.email, 'Sin información'::character varying) AS email,
    COALESCE(c.categoria, 'Sin información'::character varying) AS categoria,
    COALESCE(cl.razon_social, 'Sin información'::character varying) AS razon_social,
    COALESCE(cl.ruc, 'Sin información'::character varying) AS ruc,
